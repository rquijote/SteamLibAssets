 ...
 *
 */
export function renderBGDiv(allAssets) {
 ...
 *
 */
export function renderActiveAssetsBtn(type) {
 ...
 *
 */
export function renderAssetsGrid(fetchData, type, appId) {
    renderImages(fetchData, type);
    renderPaginationBtns(fetchData, type, appId);
}
function renderImages(fetchData, type) {
...
function renderPaginationBtns(fetchData, type, appId) {
    ...
    // Ellipsis if needed
    if (pageNum > beforeFirstPage)
        paginationUl.appendChild(createEllipsisLi());
...
function appendPaginationUl(paginationUl, type, appId) {
